            ...
            <div className='block max-w-[1024px] w-full mx-auto h-full mt-[55px]'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-3 px-4 gap-x-6 gap-y-4 mt-[50px] justify-center'>
                    {data.prismicHome.data.banner_widgets.map((val: any, i: number) => {
                        return (
                            <Link to="/" className='flex float-none bg-orange-50 md:bg-transparent px-2 items-center rounded-md cursor-pointer' key={val.widget_title + i}>
                                ...
                                <div className='float-right max-w-[25ch]'>
                                    <strong className='text-orange-500'>{val.widget_title.text}</strong>
                                    <p className='hidden md:block leading-none text-sm'>{val.widget_description.text}</p>
                                </div>
